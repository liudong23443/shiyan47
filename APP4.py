            ...
            try:
                ...
                try:
                    with st.spinner("正在生成SHAP解释图..."):
                        ...
                        # 尝试直接添加Microsoft YaHei字体到字体管理器
                        msyh_path = r'C:\Windows\Fonts\msyh.ttc'
                        if os.path.exists(msyh_path):
                            try:
                                # 注册微软雅黑字体
                                custom_font = fm.FontProperties(fname=msyh_path)
                                fm.fontManager.addfont(msyh_path)
                            except Exception as font_error:
                                st.warning(f"注册字体失败，将使用默认字体: {str(font_error)}")
                                custom_font = fm.FontProperties(family='sans-serif')
                        else:
                            custom_font = fm.FontProperties(family='sans-serif')
                            
                        # 强制使用matplotlib设置中文字体
                        ...
                        for obj in fig.findobj(mpl.text.Text):
                            obj.set_fontproperties(custom_font)
                        
                        # 手动修改标题 - 使用中文字体
                        plt.title("特征对预测的影响", fontsize=12, fontproperties=custom_font, fontweight='bold', color='black')
                        
                        # 调整坐标轴标签字体
                        ax.set_xlabel(ax.get_xlabel(), fontproperties=custom_font)
                        ax.set_ylabel(ax.get_ylabel(), fontproperties=custom_font)
